...
学习科学对 desirable difficulties 的结论是：让学习感觉轻松的手段大多损害长期保持，让检索变费力的手段大多增强它。由此五条立场：

1. 不做摘要器。"文件 → 摘要 → 闪卡"是立项时明确避开的形态。
2. 学习者的重建是核心学习行为，从节到链：隐藏参考，用自己的话解释概念、说明关系；先预测，后揭示；结课时只拿概念名、不看标题和顺序，重建概念之间的关系，对照参考图找漏掉和反了的边。结构在零基础时先由系统给出——新手需要先看到结构（expertise reversal）：这类内容单独成节（`kind: structure / process`），不套问题链的格式，因为为一个名词虚构设计理由没有真实答案；它的图与步骤表学习期间随时可回查，验收时合上，按题目给的一条路径自己讲一遍，不要求复述全图。有基础时先画再按差异学，记录未实现。
3. 高层理解不可传递，系统只创造条件：不展示 rationale 与 principle，靠顺序、问题和重复遇见让学习者自己到达。
4. 学习效果优先于图谱完整和自动化程度。
5. 理解在对比里发生，不在单例里：概念带两个表面不同、结构相同的案例时，先摆案例请学习者写共同点，再命名与揭示机制；追问和复习题优先放在最易与近邻混淆、最易诱发旧直觉的地方（Gick & Holyoak 1983；Gentner, Loewenstein & Thompson 2003；Schwartz et al. 2011；Shtulman & Valcarcel 2012）。
...
**复习课。** 开着知识库学完的课可以再建一门 `shape: review` 的课：`review_outline.py` 从掌握状态取薄弱项——假性掌握、最近判定不是 `mastered` 的概念、过期概念、错误命题、链重建漏掉的边——按被复习课的节归簇，一簇一节（`review_kind: repeat`，`parent_section` 指回原节），问题放在新情境里，先作答再揭示，节文档不得重复原节的方案与机制（校验器 `--reviewed` 做反向泄漏检查）。每个核心概念至少两次延迟证据且到过本质层的节，可以长出一个子节（`review_kind: deepen`），一门复习课最多一个，学不学由学习者在大纲确认时决定。复习不按固定间隔提醒；复习模式（间隔、范围、侧重）记录未实现。
...
一节写成什么，看学习者在这一段遇到的哪种困难，不是深浅等级：说不出名词在系统里的位置，就先安放部件（结构讲解节）；名词认识而说不出一次运行怎么走，就追踪一种明确的运行方式（过程讲解节）；有真实的机制、限制或取舍，才用问题链。三者在一门课里交替，按 `depends_on` 排序。结构讲解节与过程讲解节没有方案、没有引出的新问题；它们的预测是"先排位置"和"先排顺序"，判定分别看定位／职责／连接（`fact` 层）与步骤／参与者／变化（`mechanism` 层）。
...
- **预测**只为激活思考，不判分、不记录。本节核心概念带 `cases` 时，预测之后、揭示之前先并列两个案例请学习者写共同结构；带 `contrast` 时揭示后点出最易混的近邻与差在哪个变量。
- **主问题**要求两到三项：用自己的话解释、说明前一步为什么不足、描述输入如何变输出、指出关系与理由、给例子或失败条件、预测移除某组件的后果。不问"你理解了吗"、判断题或能从标题抄出的答案。
- **把握**三档——有把握、没把握、不知道，记录为 `confidence: 5 / 3 / 1`——是诊断信号，不参与判定。反馈优先处理有把握且错误的内容；没把握是校准信号，答对答错都说一句偏在哪；不知道但正确的明确肯定。五档在真实数据里退化成了两档（记录的值全是 4 或 5），学习者能感知的只有三态。
...
- **反馈**先指出确实正确的部分，再一次只处理一个问题：高信心误解 > 最重要的遗漏。追问针对回答实际暴露的最弱一点，从六种里选一种：边界、反例、因果深挖、删除思想实验、对比、重新归类（回答与参考冲突且两边 `ontology` 不同时——类别错置反驳无效，先换类别）。同节最多两层，之后给完整解释并要求学习者用新表述总结。
...
- **前置地图与探测轮**（骨架课）在 units 生成后、教学前。前置地图收单元正文用到而本课未定义的概念——探测题无提示、原理层、不依赖材料细节，结构上碰不到这些词；抽取总是做，诊断深度按 `prerequisite_check`，缺口按依赖类型分流。然后是探测轮：每节一道 `probe`，一次一题，一句话反馈；通过的节列为候选，全部结束后一次性问学习者跳不跳，只有学习者确认才 `defer`。
- **结课**：链重建——只给打乱顺序的概念名，学习者写成一行一条的箭头清单，抽成关系集与参考图的公开边比对（`--chain`）。比对给三个数：相邻节被连起来的比例、配上的参考边（含方向反与类型错）、精确配对；反馈用前两个，因为节链全对而每端选了别的概念时，精确那个数会接近零。不展示参考图；解释删除或替换某组件的后果；在新案例里应用同一机制；标出仍不确定的关系。快速模式只有第一项必做。总结分开报告"已解释成功""提示后成功""仍待复习""材料本身不确定"，并注明本课内的成功多为即时证据。骨架课多一步分支决策。
...
| `next_step.py`、`lesson_section.py`、`diagram.py`、`outline_status.py` | 从进度文件判断当前状态，打印该读的协议文件与下一条命令；按节打印课程数据，模型不整份读 `lesson-plan.json`；从课程数据生成问题链、系统框图与本节关系图的 Mermaid，宽度交给 `mermaid_fit.py`；每次记录后把大纲的状态列与“学习情况”块刷新到与进度文件一致 |
| `store_init.py`、`mrg_export.py`、`index_match.py`、`store_sync.py` | 知识库初始化与登记；导出公开层与高层；别名召回、登记、前置判定（`--home` 读汇总）；结课时把本目录快照推进学习者主目录并重算汇总 |
| `mermaid_fit.py` | 估算一张 Mermaid 图的宽度、按拓扑选方向、把过宽的图按固定顺序收窄；也可当命令行过一遍手写的图 |
...
| `scan_wikilinks.py` | 找未解决的 `[[双链]]`（clarify）并按节归组，一个概念为孤立、两个以上为成簇；给工作区时只扫最近更新过进度的那门课，其他课只报 inbox 计数 |
| `evals/score_pack.py` | 给一个课程目录打分：校验错误、概念数、support 分布、locator 命中率、角色与覆盖表指标、每节耗时与 `depth_reached` |

**图的宽度由代码决定。** 一张图的宽度不是作者写下的东西，是布局算法对拓扑的副作用：横排的宽度随链路深度增长，竖排随最大扇出增长；边标签在横排里逐条累加进列间距；子图一旦有边跨出边框，Mermaid 就忽略它内部的方向声明，边框还白占一个层级。模型看不见这些，所以排版不交给模型：`diagram.py` 先写出内容，再由 `mermaid_fit.py` 比较两种方向取窄的那个，仍然超过图宽预算就依次折节点名、折边标签、剥掉与端点重复的标签、把有跨界边的子图画成盒子加虚线（包含关系不丢）。真实数据：16 门真实课程的 32 张图原先 44–757 列，18 张超过预算；改后 28–96 列，无一超出。手写的图用同一个脚本过一遍。
...
**测试与评测。** 122 个单元测试；CI 跑测试、校验模板与示例包、知识库全链路冒烟。`plugin/evals/` 固定三份材料，`score_pack.py` 打分；提示词改动要能在同一组材料上看到指标变化。纯标准库 Python，无服务端。
...
规划管线与建课在 Claude Desktop 和 DeepSeek Harness 上各跑过多次；逐节教学在日常使用中。知识库模式在一个统一布局的总目录里用到 2026-09-13：库里登记了五门课——两门骨架课（一门学完，一门只跑了探测轮）、一门线性课和它的两门前置课（都已结课）；51 条学习记录，重建掌握状态后只有一条延迟证据，其余都是同日作答。未经验证的假设列在 [evidence.md](evidence.md)。结构讲解节与过程讲解节只在隔离夹具上验证过：一门三种节并存的课通过校验，系统图把 CUDA 运行时画在应用进程内部、把远程过程调用画成一条连线的说明、把统一内存留在图外作为另一种运行方式，时序图只追踪被选中的那一次运行；真实知识库里的 15 份既有计划用新校验器全部通过。还没有一门课用它们从头教到结课。

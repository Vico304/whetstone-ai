...
2. 学习者的重建是核心学习行为，从节到链：隐藏参考，用自己的话解释概念、说明关系；先预测，后揭示；结课时只拿概念名、不看标题和顺序，重建概念之间的关系，对照参考图找漏掉和反了的边。结构在零基础时先由系统给出——新手需要先看到结构（expertise reversal）；有基础时先画再按差异学，记录未实现。

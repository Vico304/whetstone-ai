...
- **结课**：链重建——只给打乱顺序的概念名，学习者写出谁引出谁、谁依赖谁，抽成关系集与参考图的公开边比对（`--chain`），反馈只说漏了哪两个概念之间的关系、哪条方向反了，不展示参考图；解释删除或替换某组件的后果；在新案例里应用同一机制；标出仍不确定的关系。快速模式只有第一项必做。总结分开报告"已解释成功""提示后成功""仍待复习""材料本身不确定"，并注明本课内的成功多为即时证据。骨架课多一步分支决策。

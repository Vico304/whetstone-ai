...
**落点厚度**：有落点不等于那里有讲解。给了 `--sources-root` 时校验器再看一遍每个 `{path, locator}`，两种情况各报一条 `WARNING: thin anchor <id> -> <path> "<locator>"`——落点在数据或代码文件里（定位到的是键名或符号名），或定位处的段落不足 200 个非空白字符（定位命中标题时取到下一个同级或更高级标题为止，否则取到空行为止；定位找不到也算）。每节另有一行 `INFO: <节> k/n concepts on thin anchors`。同样不设阈值、不改退出码：补材料还是照建由学习者定，`outline.md` 把这些行写在"落点比例"下方。判据只有这两条——概念名是否出现在段落里不能用，材料与概念名常常不同语言，真实数据上会把来源充足的课程整门判薄。
...
在大纲确认、`units/` 生成、`learning_state.py init` 之后，教学之前依次运行。先做前置地图：从 `units/` 正文取出用来解释机制、而本课 `concepts[]` 没有定义的概念（这一步总是做，空集则跳过），按 `prerequisite_check` 决定诊断深度——`always` 直接诊断，`skip` 只列清单，`auto` 列清单并问一句；要诊断时按前置检查协议的阶段一、二处理，非 `ready` 的簇按依赖类型分流，建了前置课时状态是 `BLOCKED`，回程后再探测。探测轮本身量的是原理层的地板，与前置地图不重复出题。骨架课同样可以有结构讲解节（`kind: structure`，schema 1.6）：它安放过的部件不进前置地图，那一节负责讲它们的位置。结构讲解节**没有探测题**：它的答案是系统图，而 `outline.md` 已经印着这张图；校验器不再要求它有 `probe`，写了报错。过程讲解节的探测题是排序题（打乱参与者、请学习者排先后），问题链节不变。

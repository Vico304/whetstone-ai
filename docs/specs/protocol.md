...
| 2 材料评估 | `survey_materials.py` 只读目录、文件名、大小、日期和文本前 4 KB。四级 A / B / C / D；B 与 C 的边界必须问学习者；会话记录只有学习者本人参与过才算材料，且仍是 C 级。问目的：学上游、巩固自己做的、接手别人的——决定哪一级进课程。切分的数字（≤ 30 KB、4–8 节等）是建议值，不进校验器；同一内容多个版本只取一个。另判 `subject`：材料讲的东西有没有设计者——有人做过设计决定的是 `designed`，自然界本来就有的现象是 `natural`；从扫描信号与首标题判断，判不准才问，写进档案 |
...
`prerequisite_check`：`auto`（默认，材料依赖了课程内不解释、且会阻断主线的知识，而学习者背景未知时才做）、`always`（学习者自述基础薄弱时档案里记这个）、`skip`。骨架课把这一阶段推迟到 `units/` 生成之后：先为单元正文用到而本课未定义的概念做前置地图（总是抽取，诊断深度按上面三档），再做探测轮；两者暴露地板太低时进入同样的诊断与建课。三个触发点：前置检查或探测轮之后、大纲确认时学习者说"我不懂 X、Y"、正课中途暴露缺口（只诊断受影响的簇）。
...
**PREDICT**：收到预测后，本节 core 概念带 `cases` 时先并列两个案例请学习者写共同结构，再呈现 `solution` 与 `mechanism`，一两句对照异同；带 `contrast` 的概念揭示后点出近邻与差在哪个变量。预测与共同结构不判分、不记录。结构讲解节（`structure`）改为先给部件名单、请学习者说谁包含谁与谁连谁再揭示部件图，过程讲解节（`process`）先给参与者、请学习者排步骤顺序再揭示步骤表；学习者对这一块完全没有相关知识时跳过 PREDICT——没有可激活的东西时，预测只会让他编一个答案。本节概念与前一门课的概念之间有关系时（`lesson_section.py` 打出的"跨课关系"），在揭示之前先问这条关系，一次一条，对照它的 `type` 与 `rationale` 判，记 `--kind transfer --concept <他课 id>`，不加 `--progress`，同一条只问一次。`tradeoffs` 与 `new_problem` 留作主问题和追问的素材。
...
**ASSESS**：按 `criteria` 的含义判，不看措辞；`criteria` 的含义由节类型决定——structure 节判定位、职责、连接（`fact` 层，对照 `system_map`，只判题目给的那一条路径），process 节判步骤、参与者、变化（`mechanism` 层，对照 `steps[]`），chain 节不变；分别看核心问题是否识别、机制是否因果连贯、前后关系是否正确、边界或代价是否理解、有无高信心误解、新增联系是否有依据。`verdict`：`mastered`（核心概念与关系已解释清楚）、`partial`（主线对但缺一项重要机制或关系）、`retry`（有会影响后续理解的误解）、`skipped`。`depth_reached` 与 verdict 分开：取回答自发到达的最高层，追问引导后才到达的记在追问那次；同时记满足的 criteria id。怀疑参考本身有错时暂停判分，回到来源，标为材料或参考不确定。开启知识库时判定前先写抽取 JSON，`lrg_record.py append --extraction` 调用比较器，按 `feedback_priority` 的顺序反馈；命题匿名化只去掉说话人，不放宽对象——回答里限定的对象不得换成更宽的对象，否则误解随对象一起消失。

**反馈**：先指出确实正确的具体内容；一次只处理一个问题，高信心误解优先于最重要的遗漏；给一个最小提示、反例或证据定位，或从六种追问里选一种——边界（"什么情况下 X 会失效"）、反例、因果深挖（"A 导致 B，中间发生了什么"）、删除思想实验（"把 X 拿掉哪里最先出问题"）、对比（"X 和 Y 都能做到，材料为什么选 X"）、重新归类（"X 不是一个东西，是一个过程——按过程再说一遍"，只在回答与参考冲突且两边 `ontology` 不同时用）。追问由本次回答驱动，不照抄题库；同节最多两层，之后给完整解释并要求学习者用新表述总结。同节不重复出相似题：要再验证就换更高认知层级的问题，或留给 resume 的变式题。低信心但正确的明确肯定。缺口落在本课没有定义的概念上并当场补讲时，`index_match.py recall` 取到 id 后记一条 `--kind supporting --concept <id> --verdict partial|retry --depth fact`，不加 `--progress`，同一会话同一概念只记一次；没有命中索引就不记。
...
**前置地图与探测轮**（骨架课）：见 [domain-skeleton.md](domain-skeleton.md) §5。

**结课**：先链重建——只给 `lesson_section.py --final` 打出的打乱概念名，学习者写成一行一条的箭头清单 `A → B（引出 / 依赖 / 属于同一节）`，不接受成段叙述；抽成 `relations[]` 记 `lrg_record.py append --kind final --chain`。比较器给三个数：`section_chain.ratio`（相邻节被连起来的比例，不论选哪个概念作代表）、`pair_ratio`（配上的参考边，含方向反与类型错）、`ratio`（精确）。反馈用前两个，精确只记录——节链全对却因为每端选了别的概念而拿零分，是这门课真实发生过的事。不展示参考图；课程有结构化 `system_map` 时再做一次路线重建——给一个起点和一个终点，学习者合上图说出中间经过的部件、谁装在谁里面、每条连线走什么，只判这一条路径，记 `--kind transfer --concept <路径上的部件 id>`；再解释删除或替换某组件的后果；在新案例里应用同一机制；标出仍不确定的关系。快速模式第一项必做，标准是"能把节之间的问题 → 方案串起来、说出关键取舍的方向"，链重建比例只记录不设阈值，其余可选，并写明略过的节。总结分开报告"已解释成功""提示后成功""仍待复习""材料本身不确定"，注明本课内的成功多为即时证据。开启知识库时迁移题记 `--kind transfer`，结束后 `learner_state_build.py build`。骨架课多一步分支决策。

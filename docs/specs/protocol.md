...
**PREDICT**：收到预测后，本节 core 概念带 `cases` 时先并列两个案例请学习者写共同结构，再呈现 `solution` 与 `mechanism`，一两句对照异同；带 `contrast` 的概念揭示后点出近邻与差在哪个变量。预测与共同结构不判分、不记录。structure 节改为先给部件名单、请学习者说谁包含谁与谁连谁再揭示部件图，process 节先给参与者、请学习者排步骤顺序再揭示步骤表；学习者对这一块完全没有相关知识时跳过 PREDICT——没有可激活的东西时，预测只会让他编一个答案。本节概念与前一门课的概念之间有关系时（`lesson_section.py` 打出的"跨课关系"），在揭示之前先问这条关系，一次一条，对照它的 `type` 与 `rationale` 判，记 `--kind transfer --concept <他课 id>`，不加 `--progress`，同一条只问一次。`tradeoffs` 与 `new_problem` 留作主问题和追问的素材。

...
装好后在对话里输入 `/`，看到 `guide / outline / map / learn / clarify` 五个技能即可。系统写出的一切都放在材料目录下的 `whetstone/` 里，你的材料本身不会被改动。
...
| **map** | 给一门课建立或修订系统结构图（部件、包含关系、带说明的连线），把某一节改成先安放部件或跟着一次运行走一遍，确认后写回 | 名词太多不知道各自在哪；想先看系统由什么组成 |
| **clarify** | 把你在讲义里标了 `[[双链]]` 的概念按节整理成一份解释：个别概念只解释它，一节里多个都不懂就据此把这一节重讲一遍；有来源，Obsidian 可直接打开 | 遇到不懂的概念 |

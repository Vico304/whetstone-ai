...
2. **`lesson-plan.json`**：`schema_version: "1.6"`、`shape: linear`、`prerequisite_of: <父课 lesson_id>`、`blocked_at: <父课被卡住的节 id>`、`depth: <父课 depth + 1>`（主课为 0）；`mode` 与父课相同；课程目录与父课同级（`<计划目录>/<lesson_id>/`），id 建议 `<主题>-<depth>`，如 `linear-algebra-min-1`。

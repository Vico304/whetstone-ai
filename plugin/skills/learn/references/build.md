...
**问题链怎么推进，看档案的 `subject`。** `designed`（材料讲的是人做出来的东西，有人做过设计决定）：上面五问照用——限制是什么、方案是什么、又暴露了什么代价。`natural`（讲的是自然界本来就有的现象，没有设计者）：改成**要解释哪个现象 → 哪个模型能解释它、用了什么简化 → 还有哪些特征没解释 → 需要加入什么因素或调整什么假设 → 什么证据能区分这两种解释**。后者不是每节必填的格式，是这类材料的推进方式。

自然现象没有设计者，所以**禁止为名词虚构设计理由**，也**禁止为了串起课程虚构理论发展史**，更不能把耦合关系强行排成单向因果链——"顺序讲解"不等于"真实发生的时间顺序"。

`subject: natural` 时另有两条：材料里有多种解释的地方，用 `tradeoffs[].contested` 与两方来源如实记下，不挑一个当标准答案，判不了的进 `uncertainties`；成熟理论与必要模型**可以直接讲**——它们是机制层，本来就可以展示，不要因为"听起来像原理"藏进高层。本质层在这类材料里是"为什么用这个模型而不是那个、什么证据能区分"，仍然不展示。
...
- `outline.md`：学习目标与模式、材料范围、总体问题、系统地图（`diagram.py --system` 的框图）、问题链（每 unit 一行问题 → 方案，加 `diagram.py --chain` 的框图，校验器检查它覆盖每个未略过的 unit）、**全部概念按 unit 与角色列出**、覆盖账本摘要、使用说明。不含任何 unit 的机制、意义、代价、criteria、principle。
...
- "新问题"应自然引出下一 unit；最后一个 unit 可转为未决问题、边界或迁移挑战。图不强制：本节概念的关系图用 `diagram.py --section <id>` 生成；机制或架构的框图（框套框、数据流）有助于理解时按材料手写 Mermaid，只画公开层的东西，写完用 `scripts/mermaid_fit.py <文件> --in-place --report` 过一遍。手写时守四条，它们决定图有多宽：语义能从两端节点名读出来的边不加标签；必须加的标签用 `<br/>` 自己折行，不靠 `wrap`；不画有边跨出边框的子图——要么让子图封闭，要么拆成两张；一张图只画一层，内部结构另写一篇用 `[[双链]]` 串过去，两张各自看得清的图永远好过一张要缩放的大图。

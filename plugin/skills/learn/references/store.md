...
- build 校验通过后：`scripts/mrg_export.py <lesson-plan> --store <工作区>/store`，得到公开层 `mrg/<id>.json` 与高层 `mrg/<id>.deep.json`；随后 `scripts/index_match.py register --store <工作区>/store --lesson-id <id>` 把节点登记进本地注册表（脚本报告的 alias 冲突不自动处理，交学习者确认）；再 `scripts/cards.py build --store <工作区>/store --lesson-id <id>` 为事实层与 `listed` 概念生成事实卡（只有这两类，不给机制、本质、思想做卡）；
...
- 教学中每次产生 verdict：按 [protocol/assess.md](protocol/assess.md) 写抽取 JSON，用 `scripts/lrg_record.py append --store <工作区>/store --progress <learning-progress.json>` 一次完成比较、追加日志、同步进度；

...
- 手写的图有了约定，渲染侧有了兜底。建课协议里"按材料手写 Mermaid"那条补四件事：语义能从两端节点名读出来的边不加标签；必须加的标签用 `<br/>` 自己折行，不靠 `wrap`；不画有边跨出边框的子图，要么让子图封闭要么拆成两张；一张图只画一层，内部结构另写一篇用 `[[双链]]` 串过去——两张各自看得清的图永远好过一张要缩放的大图。写完用 `mermaid_fit.py <文件> --in-place --report` 过一遍。概念解释的笔记模板多一节"内部结构"，同样只画一层。渲染侧只需要一条保底规则，随技能附一份 Obsidian 代码片段 `assets/obsidian-mermaid.css`：给 svg 写 `max-width: 100%` 与 `height: auto`，**不要写 `width`**——写 `width: 100%` 会把本来就窄的图反向拉大到糊掉；确实需要横向空间的笔记在 frontmatter 里写 `cssclasses: wide-diagram`，能多出四到六成横向空间。节点名的折行阈值从 8 个全角字符放宽到 12：在 15 门真实课程上两个阈值都让全部图落进预算，宽的那个折得更少。

- 图的宽度改由代码决定。此前四处生成 Mermaid 的地方方向都是写死的，其中两处写反了：旧的步骤列表形式的系统图横排，本节关系图横排。横排的宽度随链路深度增长，一串步骤横着排就是最坏情况——15 门真实课程的系统框图是 114–757 列（按等宽列数算，全角字符算两列），压进笔记宽度后只能整体缩小到看不清，而渲染侧只有等比缩放这一个自由度，在那边无解。新脚本 `mermaid_fit.py` 估算一张图在两种方向下各有多宽，生成时取窄的那个；仍然超过图宽预算（默认 96 列）就依次折节点名、折边标签、剥掉与端点重复的边标签、把有跨界边的子图画成盒子加虚线——Mermaid 一旦有边跨出子图边框就忽略它内部的方向声明，那圈边框只剩下白占一个层级的作用。改后同样这 15 门课是 20–96 列，全部竖排。顺带修掉另外两处：过程讲解节的时序图用 `Note over` 而不是 `Note right of`（后者从最右边的参与者再往右伸一大截，是时序图最大的宽度来源），本节关系图的边标签从字段值 `depends_on` 改成"依赖"这类中文词。`mermaid_fit.py` 也可以当命令行用，把手写的图、或一篇笔记里的每个 Mermaid 块过一遍（`--report` 说明改了什么、宽度从多少降到多少）；它只认 flowchart 与 sequenceDiagram，认不出的行原样保留并列进报告，不做完整的 Mermaid 解析器。
